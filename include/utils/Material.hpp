...
/*
    ...
    - Desc:     describes a "look" effect applied to an element's background.
                ...
                --- Each kind is exposed as a *static factory* that returns a
                Material by value, and Material itself has chainable setters
                that return `Material&`. This lets you tweak any preset inline
                at the call site, the same way `Modifier` and `*Options` work
                elsewhere in UILO:   Pick a preset and override two fields: row(
                Modifier() .setMaterial( Material::Blur() .setOpacity(0.4f)
                ...
                (animated) - Aurora       soft drifting magenta/teal colour
                clouds (animated) - Tinted       glass that uses the element's
                own colour as tint - Ripple       Tinted + concentric ripples
                emanating from the cursor - Hover        Tinted + radial
                highlight under the cursor - Blur         semi-transparent blur
                of the backdrop, tinted with the element's colour. Per-element
                blur radius.
*/
