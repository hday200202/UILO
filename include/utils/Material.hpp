...
/*
    Material:
    - Desc: describes a "look" effect applied to an element's background.
            Materials are carried on a Modifier (`Modifier::setMaterial`).
            Containers (Row/Column) and interactible widgets (Button,
            Dropdown) honour the material; everything else ignores it. API ---
            Each kind is exposed as a *static factory* that returns a Material
            by value, and Material itself has chainable setters that return
            `Material&`. This lets you tweak any preset inline at the call
            site, the same way `Modifier` and `*Options` work elsewhere in
            UILO:   Pick a preset and override two fields: row( Modifier()
            .setMaterial( Material::Blur() .setOpacity(0.4f)
            .setBlurRadius(12.f)), ...);   Start from scratch (kind = None,
            no effect): auto m = Material() .setKind(Material::Kind::Glass)
            .setTint({200, 220, 255, 40}) .setOpacity(0.6f); Available kinds
            --------------- - None         no effect - Glass        Apple-
            style frosted glass: backdrop blur + tint + lens - Frosted
            denser, more matte variant of Glass - Holographic  iridescent
            shifting tint (animated) - Liquid       sinusoidal UV ripples
            (animated) - Shimmer      Glass + diagonal highlight sweep
            (animated) - Aurora       soft drifting magenta/teal colour clouds
            (animated) - Tinted       glass that uses the element's own colour
            as tint - Ripple       Tinted + concentric ripples emanating from
            the cursor - Hover        Tinted + radial highlight under the
            cursor - Blur         semi-transparent blur of the backdrop,
            tinted with the element's colour. Per-element blur radius.
*/
...
struct Material {
    ...
    /* fields */
    Kind  kind          = Kind::None;
    /* Tint applied on top of the blurred backdrop. */
    ...
    /* Body opacity (0..1). Lower = more of the underlying scene shows through.
       Apple-style glass is around 0.55..0.75. */
    float opacity       = 0.65f;
    /* Edge refraction strength: how strongly the blurred backdrop is pulled
       toward the centre near the rounded-rect boundary, in pixels. */
    float refraction    = 28.f;
    /* Animation tempo (cycles/sec-ish) and amplitude for *animated* kinds. */
    float animSpeed     = 1.0f;
    float animStrength  = 1.0f;
    /* Extra blur radius (in pixels) applied on top of the global blur ladder.
       Only consumed by the Blur kind. */
    ...
    /* Blur: semi-transparent blur of what's behind the element, tinted with
