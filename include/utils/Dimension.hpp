...
/*
    ...
    - Desc:     A size given either as absolute pixels or as a percentage of
                whatever the parent offers. The `percent` flag says which, and
                resolve() turns the pair into a concrete number against a parent
                size. Percent children share the space left over after their
                fixed-size siblings have taken theirs, which is why the two are
                kept distinct rather than resolved up front.
