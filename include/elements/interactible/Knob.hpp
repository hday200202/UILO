...
/*
    ...
    - Desc:     Storage signature for a knob's value callback, fired only when
...
/*
    ...
    - Desc:     Which way the arc sweeps from the start angle to the end angle.
                Together with those two angles this covers every knob layout:
                from 135 to 45 clockwise sweeps under the bottom, and the same
                pair counter-clockwise sweeps over the top.
*/
...
/*
    ...
    - Desc:     Everything a Knob draws and the range it works over: the body
                and its rim, the unfilled track, the filled arc, the indicator
                line, the arc's geometry, and the value range and step. Colors
                come as a literal plus a role, where the role wins when it
                resolves against the active Palette and the literal is the
                fallback.
...
/*
    Knob:
    - Desc:     A rotary value control. Dragging vertically changes the value,
                at a rate set by dragPixelsPerRange, which is the convention a
                mouse can actually work with -- following the pointer around the
                circle makes a small knob unusable. The wheel adjusts it too,
                accumulating sub-step motion so a stepped knob still responds to
                a slow trackpad, and a double click restores the configured
                default.
