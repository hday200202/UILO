...
Corner radius, resolved in three steps: the value this element was given, then the active Theme's, then the fallback carried by inheritRounding (0 for a plain [Column](#column)). Resolved on every read rather than cached, so changing the Theme restyles an element already on screen.
